function y = moc_prepad (x, l, c, dim)
// prepend the scalar
// Calling Sequence
// y = moc_prepad (x, l,c)
// y = moc_prepad (x, l,c,dim)
// Description
// Prepend the scalar value c to the vector x
...
// Created: June 1994
  ...
  if (nargin < 3 | isempty (c))
    ...
    if (~sum(length(c))==1)
      error ("moc_prepad: third argument must be empty or a scalar");
    end
  end
  ...
  if (nargin < 4)
    ...
    if (~(sum(length(dim))==1 & dim == round (dim)) & dim > 0 & dim < (nd + 1))
      error ("moc_prepad: dim must be an integer and valid dimension");
    end
  end
