...
// credited to Randolph Franklin.
 ...
 [nargout,nargin]=argn(0);
  ...
  if (~ (isreal (x) & isreal (y) & ismatrix (y) & ismatrix (y)  & sum(size (x)-size(y))==0))
    error ("inpolygon: first two arguments must be real matrices of same size");
  elseif (~ (isreal (xv) & isreal (yv) & isvector (xv) & isvector (yv)   & sum(size (xv) - size(yv))==0))
  ...
  for i = 1 : npol
    ...
    idx1 = (((yv(i) <= y & y < yv(j)) | (yv(j) <= y & y < yv(i)))  & 0 < distance.*delta_yv);
    in (idx1) = ~in (idx1);

    // Check if (x,y) are actually on the boundary of the polygon.
    if (do_boundary)
       idx2 = (((yv(i) <= y & y <= yv(j)) | (yv(j) <= y & y <= yv(i)))  & ((xv(i) <= x & x <= xv(j)) | (xv(j) <= x & x <= xv(i)))    & (0 == distance | ~delta_xv));
       on (idx2) = %t;
    end
    j = i;
  end
